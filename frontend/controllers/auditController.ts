...
// แถวจริงใน audit_logs มี actor_login (services/audit แนบชื่อผู้กระทำไว้ตอนเขียน)
// ซึ่งกว้างกว่า AuditLog ใน models/types.ts — ขยายชนิดเฉพาะที่นี่
type AuditRow = WithId<AuditLog> & { actor_login?: string | null };
...
export async function listAudit(user, url) {
  ...
  const items: AuditRow[] = await AuditLogs.find(f, { sort: { at: -1 }, limit });
