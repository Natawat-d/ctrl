...
export async function createUnit(user, body) {
  ...
  // zone_id/floor ที่ยังไม่มีค่าเก็บลง MongoDB เป็น null (ตามของเดิมสมัย JS)
  // ส่วน Unit ใน models/types.ts ประกาศไว้เป็น optional — cast ให้ตรงชนิดโดยไม่แตะค่าที่เขียนจริง
  const unit = await Units.insert({
    tenant_id, market_id: marketId, zone_id: (oid(body.zone_id) || null) as Unit["zone_id"],
    type, code, name: body.name || code,
    floor: (body.floor != null && body.floor !== "" ? Number(body.floor) : null) as Unit["floor"],
