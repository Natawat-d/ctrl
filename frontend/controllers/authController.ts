...
// เอกสาร user ที่อ่านกลับมาจาก mongo มี _id เสมอ — type กลางประกาศ _id เป็น optional
// เพราะตอน insert ยังไม่มี _id (ดู models/types.ts)
type UserDoc = User & { _id: ObjectId };
...
export async function login(req) {
  ...
  const u = (await Users.findOne({ login_id })) as UserDoc | null;
...
export async function signup(req) {
  ...
  // verify_token / accepted_terms_at มีจริงในเอกสารแต่ยังไม่ได้ประกาศใน models/types.ts
  ...
  } as User & { verify_token: string; accepted_terms_at: Date });
...
export async function forgotPassword(req) {
  ...
  const u = (await Users.findOne({ $or: [{ login_id: key }, { email: key.toLowerCase() }], role: "owner" })) as UserDoc | null;
  const resp: Record<string, any> = { ok: true, message: "ถ้ามีบัญชีนี้ ระบบได้ส่งลิงก์ตั้งรหัสใหม่ไปที่อีเมลแล้ว" };
...
export async function resetPassword(req) {
  ...
  if (String(password).length < 6) return httpError(400, "weak", "รหัสผ่านอย่างน้อย 6 ตัว");
  const u = (await Users.findOne({ reset_token: String(token) })) as UserDoc | null;
