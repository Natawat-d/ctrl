...
export async function GET(req: Request) {
  ...
  const f: Record<string, any> = { role: "tenant_user" };
...
export async function POST(req: Request) {
  ...
  let market: WithId<Market> | null = null;
  if (unitOid) {
    const unit = await db.collection("units").findOne({ _id: unitOid });
    if (unit) market = await db.collection<Market>("markets").findOne({ _id: unit.market_id });
  }
  if (!market && tid) market = await db.collection<Market>("markets").find({ tenant_id: tid }).sort({ market_short: 1 }).limit(1).next();
  ...
  const doc = {
    tenant_id: tid, role: "tenant_user", login_id: finalLogin, password_hash: hashPassword(password),
