...
export default function AdminPage() {
  ...
  const [pwFor, setPwFor] = useState(null); const [newPw, setNewPw] = useState(""); const [pwBusy, setPwBusy] = useState(false); const [copied, setCopied] = useState(false);
  const [showPw, setShowPw] = useState({}); // { ownerId: true } เผยรหัสเจ้าของ (platform_admin เท่านั้น)
  ...
  async function savePw() {
    ...
    try {
      ...
      setPwFor(null);
    } catch (e) { setErr(e.message); } finally { setPwBusy(false); }
  }
                ...
                <div className="card pad" key={s._id}>
                  ...
                  {o && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 18px", marginTop: 8, fontSize: 12.5, alignItems: "center" }}>
                      ...
                      <span style={{ color: "var(--muted)" }}>รหัสผ่าน: <strong style={{ fontFamily: "ui-monospace, monospace" }}>{showPw[o._id] ? (o.password_plain || "—") : "••••••••"}</strong>
                        <button className="btn ghost sm" style={{ marginLeft: 4, padding: "1px 6px" }} onClick={() => setShowPw((s) => ({ ...s, [o._id]: !s[o._id] }))}>{showPw[o._id] ? "ซ่อน" : "ดู"}</button>
                        {o.password_plain && <button className="btn ghost sm" style={{ marginLeft: 2, padding: "1px 6px" }} title="คัดลอกรหัสผ่าน" onClick={() => { try { navigator.clipboard.writeText(o.password_plain); flash("คัดลอกรหัสผ่านแล้ว"); } catch {} }}>คัดลอก</button>}
                      </span>
