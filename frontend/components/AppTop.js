...
export default function AppTop({ title, sub, actions, children }) {
  const path = usePathname();
  const router = useRouter();
  const dispatch = useDispatch();
  const user = useSelector(selectUser);
  const markets = useSelector((s) => s.markets.items);
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        <div className="brand"><span className="logo">CT</span><span>CTRL</span></div>
