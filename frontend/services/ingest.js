...
export async function ingestTelemetry(t) {
  ...
  const set = { "status.online": true, "status.last_seen": now, updated_at: now };
  ...
  await db.collection("meters").updateOne({ _id: dev.meter_id }, { $set: { last_value: lastValue, last_ts: ts } });
  return { ok: true };
}
